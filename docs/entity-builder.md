...
export const authOperators = {
   hash: colOp((algo: "bcrypt" | "argon2") => ({
      ...
      fields: {
         ...
         update: ax.string().transform(v => hash(v, algo)).optional(),
      },
   })),
   userId: colOp({
      ddl: { type: "int" },
...
export const users = entity({
   ...
   ].createdAt().updatedAt(),   // ← or as standalone columns:
   ...
   constraints: (col, ctx) => [
      ctx.unique([col.role, col.email]),
      ctx.index([col.role, col.score]),
      ctx.checks([col.score.gte(0)]),
   ],
