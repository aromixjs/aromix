...
export class Parser<Output> {
    ...
    switch (this.state.type) {
      ...
      case 'date': {
        if (!(value instanceof Date) && typeof value !== 'string') {
          throw this.makeError({ code: 'invalidType', path, expected: 'date', received: typeof value })
        }
        if (typeof value === 'string') {
          const d = new Date(value)
          if (isNaN(d.getTime())) {
            throw new ValidationError([
              { code: 'invalidDate', path, message: `Invalid date string "${value}"` },
            ])
          }
          return d
        }
        if (isNaN(value.getTime())) {
          throw new ValidationError([
            { code: 'invalidDate', path, message: 'Invalid Date object' },
          ])
        }
        return value
      }
      case 'literal': {
        if (value !== this.state.literal!.value) {
          ...
          throw new ValidationError([
            { code: 'invalidLiteral', path, message: `Expected ${expected}, received ${received}` },
          ])
        }
        return value
      }
  ...
  private makeError(opts: {
    ...
    return new ValidationError([
      { code: opts.code, path: opts.path, message: `Expected ${opts.expected}, received ${opts.received}${p}` },
    ])
  }
}
